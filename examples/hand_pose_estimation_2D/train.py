...
from paz.processors import TRAIN
from paz.models import KeypointNet2D

from freihandv2 import FreiHANDV2
...
parser.add_argument('-nk', '--num_keypoints', default=21, type=int,
...
data_manager = FreiHANDV2(path, TRAIN)
data = data_manager.load_data()

# split training data-set into train and validation
num_train_samples = int(len(data) * (1 - args.validation_split))
datasets = {
    'train': data[:num_train_samples], 'validation': data[num_train_samples:]}

# instantiate keypoint augmentations
processor = {}
for split in ['train', 'validation']:
    processor[split] = PreprocessHandPose(
        (args.image_size, args.image_size), split)

# creating sequencers
sequence = {}
for split in ['train', 'validation']:
    pipeline, data = processor[split], datasets[split]
    sequence[split] = ProcessingSequence(pipeline, args.batch_size, data, True)
...
model_name = ['HandKP', model.name, str(args.filters), str(args.num_keypoints)]
